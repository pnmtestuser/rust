...
pub(super) fn mangle(
    ...
    let print_symbol = |mut cx: SymbolMangler<'_, 'tcx>| {
        ...
        if let Some(instantiating_crate) = super::instantiating_crate(tcx, instance) {
...
struct CompressionCaches<'tcx> {
    ...
    consts: FxHashMap<&'tcx ty::LazyConst<'tcx>, usize>,
}
...
impl SymbolMangler<'_, 'tcx> {
    ...
    fn push_ident(&mut self, ident: &str) {
        ...
        if !bytes.is_empty() {
            match bytes[0] {
                ...
                b => bug!("symbol_names: bad start byte {} in ident {:?}", b, ident),
            }
            for &b in &bytes[1..] {
                match b {
        ...
        if !use_punycode {
            let _ = write!(self.out, "{}{}", ident.len(), ident);
            return;
        }
        ...
        // Replace `-` with `_`.
        let punycode_suffix = match punycode_bytes.iter().rposition(|&c| c == b'-') {
            Some(i) => {
                punycode_bytes[i] = b'_';
                &mut punycode_bytes[i + 1..]
            }
            None => &mut punycode_bytes,
        };
        // Replace [0-9] with [A-J].
        for b in punycode_suffix {
            if let b'0'..=b'9' = *b {
                *b = b'A' + (*b - b'0');
            }
        }

        let punycode_string = String::from_utf8(punycode_bytes).unwrap();
        let _ = write!(self.out, "u{}{}", punycode_string.len(), punycode_string);
    }
    ...
    // FIXME(eddyb) move this to `Printer` when const generics land.
    fn print_const(mut self, constant: &'tcx ty::LazyConst<'tcx>) -> Result<Self, !> {
        if let Some(&i) = self.compress.as_ref().and_then(|c| c.consts.get(&constant)) {
        ...
        let ty = match *constant {
            ty::LazyConst::Unevaluated(def_id, substs) => {
                self.tcx.type_of(def_id).subst(self.tcx, substs)
            }
            ty::LazyConst::Evaluated(c) => c.ty,
        };
        match ty.sty {
            ty::Uint(_) => {}
            _ => {
        ...
        match *constant {
            ...
            ty::LazyConst::Evaluated(c) => {
                let bits = c.unwrap_bits(self.tcx, ty::ParamEnv::empty().and(ty));
        ...
        if !constant.has_escaping_bound_vars() {
            if let Some(c) = &mut self.compress {
                c.consts.insert(constant, start);
            }
        }
        Ok(self)
    }
...
impl Printer<'tcx, 'tcx> for SymbolMangler<'_, 'tcx> {
    ...
    fn path_crate(
        ...
        match name.as_bytes()[0] {
            // Crate names can start with digits, so escape them with `_`
            // (and also `_` itself, to preserve the distinction).
            b'0'..=b'9' | b'_' => self.push_ident(&format!("_{}", name)),

            _ => self.push_ident(&name),
        }
        Ok(self)
    }
    fn path_qualified(
